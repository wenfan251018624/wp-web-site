...
<?php get_footer(); ?>';
    ...
    $theme_name = 'twentytwentyfour'; // 默认主题
    
    // 如果twentytwentyfour不存在，使用twentytwentythree
    if (!is_dir($theme_dir . $theme_name)) {
        $theme_name = 'twentytwentythree';
    }
    
    // 如果twentytwentythree也不存在，使用twentytwentytwo
    if (!is_dir($theme_dir . $theme_name)) {
        $theme_name = 'twentytwentytwo';
    }
    
    $template_path = $theme_dir . $theme_name . '/youtube-homepage.php';
